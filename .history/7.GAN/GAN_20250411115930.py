...
discriminator.compile(loss='binary_crossentropy', optimizer=Adam(lr=0.002, beta_1=0.5), metrics=['accuracy'])  # Fixed syntax error

generator = build_generator()

gan = build_gan(generator, discriminator)
gan.compile(loss='binary_crossentropy', optimizer=Adam(lr=0.002, beta_1=0.5), metrics=['accuracy'])
...
def train_gan(epochs, batch_size = 128):
    ...
    for epoch in range(epochs):
        for i in range(batch_count):
            ...
            d_loss_real = discriminator.train.on_batch(real_images, real_labels)
            d_loss_fake = discriminator.train.on_batch(generated_images, fake_labels)
