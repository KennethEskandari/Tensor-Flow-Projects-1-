...
model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=0.000000000001), loss='mean_squared_error')
...
#Training The Model
history = model.fit(Views, Controversy, epochs=10000, verbose=1)
...
#Our next step is evalution. 
#We want to see how well the model is doing
#and we want to see the relationship between the two variables
...
#nan - something blew up (try lowering the learning rate)
#An Interger (That is the number that is telling you how wrong the model is )

#congratulations! You now know how wrong your model is